...
number:/^[+-]?(\d+(\.\d*)?|\.\d+)([Ee]-?\d+)?$/,zip:/^\d{5}(-\d{4})?$/,phone:/^[2-9]\d{2}-\d{3}-\d{4}$/,guid:/^(\{?([0-9a-fA-F]){8}-(([0-9a-fA-F]){4}-){3}([0-9a-fA-F]){12}\}?)$/,time12:/^((0?\d)|(1[012])):[0-5]\d?\s?[aApP]\.?[mM]\.?$/,time24:/^(20|21|22|23|[01]\d|\d)(([:][0-5]\d){1,2})$/,nonHtml:/^[^<>]*$/},messages:{require:"Es requerido.",match:"No tiene un formato valido.",integer:"Debe ser positivo.",date:"La fecha debe estar en el formato (mm/dd/yyyy)",
email:"Debe ser un email valido.",usd:"Debe ser una cantidad de dinero valida.",url:"Debe tener un formato URL valido.",number:"Debe ser un numero.",zip:"#{field} must be formatted as a zipcode ##### or #####-####.",phone:"Recuerde el telefono debe ser ###-###-####.",guid:"#{field} must be formatted as a guid like {3F2504E0-4F89-11D3-9A0C-0305E82C3301}.",time24:"#{field} must be formatted as a 24 hour time: 23:00.",time12:"#{field} must be formatted as a 12 hour time: 12:00 AM/PM",
lessThan:"Debe ser menor que #{max}.",lessThanOrEqualTo:"Debe ser menor o igual que #{max}.",greaterThan:"Debe ser mayor que #{min}.",greaterThanOrEqualTo:"Debe ser mayor o igual a #{min}.",range:"Debe estar entre #{min} y #{max}.",tooLong:"No puede ser mas largo que #{max} caracteres.",tooShort:"No debe ser menor que #{min} caracteres.",nonHtml:"#{field} cannot contain HTML characters.",alphabet:"No puede contener caracteres especiales.",
minCharClass:"No puede tener mas de #{min} #{charClass} caracteres.",maxCharClass:"No debe tener menos de #{min} #{charClass} caracteres.",equal:"los valores no son iguales.",distinct:"Hay valores repetidos.",sum:"Values don't add to #{sum}.",sumMax:"The sum of the values must be less than #{max}.",sumMin:"The sum of the values must be greater than #{min}.",radioChecked:"The selected value is not valid.",generic:"Invalido."},out:{start:function(){this.defer("start")},end:function(b){this.defer("end",
